...
class PythonSandbox:
    ...
    def __init__(self):
        ...
        self.current_chi_square = 0.0  # 用于保存当前的chi-square值
        ...
        try:
            ...
            self.flight_file.write("frame_number,divergence,chi_square\n")
    ...
    def processNoUSB(self, inframe):
        try:
            ...
            # 将divergence和chi-square写入CSV日志中
            if self.flight_file:
                self.flight_file.write(f"{self.frame_number},{self.smoothed_divergence:.3f},{self.current_chi_square:.4f}\n")
                self.flight_file.flush()
    ...
    def process_frame_optical_flow(self, frame_bgr, dt):
        ...
        # 定义一个函数，用于生成图像中心区域的mask
        def create_center_mask(image):
            mask = np.zeros_like(image)
            height, width = image.shape
            center_x = width // 2
            center_y = height // 2
            # 定义ROI为图像中心区域（此处设定为图像的一半，可调整）
            roi_width = width // 2
            roi_height = height // 2
        ...
        if self.old_gray is None:
            ...
            self.p0 = cv2.goodFeaturesToTrack(
                ...
                mask=mask  # 使用中心区域mask
            )
            if self.p0 is not None:
                self.p0 = self.p0.reshape(-1, 1, 2)
            return

        # 如果特征点不可用，则重新利用中心mask检测特征
