...
class PythonSandbox:
    ...
    def process(self, inframe, outframe):
        ...
        try:
            ...
            # Multiply by 100 only when sending
            x = int(self.smoothed_divergence * 100)
            y = self.current_obstacle_flag

            # 限制 x 的范围为 -128 到 127
            if x < -128:
                self.log_warning(f"x value {x} is less than -128. Clamping to -128.")
                x = -128
            elif x > 127:
                self.log_warning(f"x value {x} is greater than 127. Clamping to 127.")
                x = 127

            # 将 x 转换为无符号字节（两补数表示）
            x_unsigned = x & 0xFF  # 保留最低8位
    ...
    def process_frame_obstacle_detection(self, frame_bgr):
        ...
        if self.initializing:
            ...
            # Add distribution to all distributions
            self.all_distributions.append(distribution)
            # Add distribution to update batch
            self.update_distributions_batch.append(distribution)

            # Perform K-Means update at specified intervals
                ...
                if min_distance_to_initial > self.BACKGROUND_DISTANCE_THRESHOLD:
                    ...
                    for existing_center in self.obstacle_centers:
                        ...
                        if distance_to_existing < self.MERGE_DISTANCE_THRESHOLD:
                            # Merge with existing obstacle center
    ...
    def detect_obstacle(self, distribution):
        # Compare distribution with initial cluster centers to determine if it's background
        ...
        # Compare distribution with obstacle cluster centers
