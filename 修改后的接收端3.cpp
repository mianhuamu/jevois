...
// Define a structure to store the parsed coordinates
typedef struct {
    float x;
    float y;
} FloatCoordinates;
...
void appMain() {
    ...
    // Register the callback function for CPX packets
...
static void cpxPacketCallback(const CPXPacket_t* cpxRx) {
    ...
    // Validate packet length (at least 5 bytes: 1 start byte + 1 length byte + 3 data bytes)
    if (cpxRx->length < 5) {
        DEBUG_PRINT("Received packet too short: length=%d\n", cpxRx->length);
        return;
    }

    // Validate if the start byte is 0xFF
    ...
    // Verify if the actual packet length matches the expected length
    if (cpxRx->length != (2 + data_length + 1)) { // Start byte + length byte + data bytes + checksum
        DEBUG_PRINT("Data length mismatch: expected %d, got %d\n", 2 + data_length + 1, cpxRx->length);
        return;
    }

    // Calculate checksum (XOR of all bytes except the last byte)
    uint8_t calculated_checksum = 0;
    for(int i = 0; i < (cpxRx->length - 1); i++) { // Exclude the last byte (checksum)
    ...
    // Parse x (signed short, big-endian)
    ...
    // Print corresponding information based on the value of obstacle
    if (obstacle == 1.0f) {
