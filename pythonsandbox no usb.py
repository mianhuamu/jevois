...
class PythonSandbox:
    ...
    def __init__(self):
        ...
        # Initialize drivers for data sending
        ...
        # Prepare output directory for divergence logs
    ...
    def processNoUSB(self, inframe):
        try:
            ...
            # Skip processing if background model is not ready
            if not self.initializing and self.background_model is None:
                self.log_warning("Background model not initialized.")
                return

            if self.last_frame_time is None:
                # For the first frame, record time and skip optical flow
                self.last_frame_time = current_time
                return
            dt = current_time - self.last_frame_time
            self.last_frame_time = current_time

            # Process optical flow with center-region feature detection
            self.process_frame_optical_flow(inimg, dt)

            # Process obstacle detection using texton distribution
            ...
            # Prepare and send data via serial port
    ...
    def process_frame_optical_flow(self, frame_bgr, dt):
        ...
        # Define a function to create a mask for the center region
        def create_center_mask(image):
            mask = np.zeros_like(image)
            height, width = image.shape
            center_x = width // 2
            center_y = height // 2
            # Define ROI as the center half of the image (adjustable)
            roi_width = width // 2
            roi_height = height // 2
        ...
        # If there is no previous frame, initialize and detect features in the center region
        if self.old_gray is None:
            ...
            self.p0 = cv2.goodFeaturesToTrack(
                ...
                mask=mask  # Use center region mask
            )
            if self.p0 is not None:
                self.p0 = self.p0.reshape(-1, 1, 2)
            return

        # If no features are available, re-detect using the center mask
        ...
        if p1 is not None and st is not None:
            ...
            if len(good_new) < 2:
                ...
                # Compute optical flow vectors and update divergence
                vectors = [
    ...
    def process_frame_obstacle_detection(self, frame_bgr):
        ...
        # In initialization phase, build background model with multiple frames
    ...
    def low_pass_filter_recursive(self, new_divergence, previous_divergence, factor, div_factor, dt):
        ...
        # Scale new_divergence by dt and div_factor
