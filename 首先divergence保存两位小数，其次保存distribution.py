...
class PythonSandbox:
    ...
    def __init__(self):
        ...
        # A list to store the smoothed divergence
        self.divergence_history = []
        # A list to store texton distributions for obstacle frames
        self.obstacle_distribution_history = []

        # Log file paths (modifiable according to your needs)
        self.DIVERGENCE_LOG_PATH =  "/jevois/data/shixu/divergence_log-test.csv"
        self.OBSTACLE_DISTRIBUTION_LOG_PATH = "/jevois/data/shixu/obstacle_distribution.csv"
        ...
        self.current_obstacle_flag = int(0)
    ...
    def process(self, inframe, outframe):
        ...
        try:
            ...
            self.log_info("Resized input image.")
            ...
            # Log divergence and obstacle flag
    ...
    def process_frame_optical_flow(self, frame_bgr, dt):
        ...
        if p1 is not None and st is not None:
            ...
            if len(good_new) < 2:
                ...
                # Obtain the smoothed divergence through low-pass filtering
                self.smoothed_divergence = self.low_pass_filter_recursive(
                    divergence, 
                    self.smoothed_divergence, 
                    self.lp_factor, 
                    self.div_factor, 
                    dt
                )

                # Save the smoothed divergence to file
                if not np.isnan(self.smoothed_divergence):
                    self.divergence_history.append(self.smoothed_divergence)
                    self.save_divergence(self.smoothed_divergence)
    ...
    def process_frame_obstacle_detection(self, frame_bgr):
        ...
        if self.initializing:
            ...
            # Add distribution to the total distribution list
            self.all_distributions.append(distribution)
            # Add distribution to the update batch
            self.update_distributions_batch.append(distribution)

            # Perform a K-Means update at the specified interval
            ...
            # Detect obstacle using K-Means clusters
            label, color = self.detect_obstacle(distribution)
            cv2.putText(frame_bgr, label, (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2, cv2.LINE_AA)

            # If an obstacle is detected, save the texton distribution for this frame
            if label == self.OBSTACLE_LABEL:
                self.save_obstacle_distribution(distribution)
    ...
    def update_kmeans(self):
        ...
        try:
            ...
            for idx, c_new in enumerate(centers):
                ...
                # Check if the new center is significantly different from initial centers
                if min_distance_to_initial > self.BACKGROUND_DISTANCE_THRESHOLD:
                    ...
                    for existing_center in self.obstacle_centers:
                        ...
                        if distance_to_existing < self.MERGE_DISTANCE_THRESHOLD:
                            # Merge with the existing obstacle center
    ...
    def detect_obstacle(self, distribution):
        # Compare the distribution with initial cluster centers to decide if it is background
        if self.initial_cluster_centers is not None and len(self.initial_cluster_centers) > 0:
            ...
            if min_distance_to_initial <= self.BACKGROUND_DISTANCE_THRESHOLD:
                self.obstacle_detected = False
                self.current_obstacle_flag = 0
                return self.NO_OBSTACLE_LABEL, (0, 255, 0)

        # Compare the distribution with obstacle cluster centers
        if len(self.obstacle_centers) > 0:
            ...
            if np.any(distances < self.DISTANCE_THRESHOLD):
                ...
                return self.OBSTACLE_LABEL, (0, 0, 255)
            else:
                self.obstacle_detected = False
                self.current_obstacle_flag = 0
                return self.NO_OBSTACLE_LABEL, (0, 255, 0)
        ...
        try:
            with open(dictionary_path, 'rb') as f:
                ...
                for _ in range(n_textons):
                    ...
                    texton_vectors.append(
                        np.hstack((texton.Y.flatten(), texton.U.flatten(), texton.V.flatten()))
                    )
    ...
    def save_divergence(self, divergence_value):
        try:
            with open(self.DIVERGENCE_LOG_PATH, "a") as f:
                timestamp = time.time()
                f.write(f"{self.frame_number},{divergence_value:.2f},{timestamp:.2f}\n")
        except Exception as e:
            self.log_error(f"Failed to save divergence to file: {e}")

    def save_obstacle_distribution(self, distribution):
        try:
            with open(self.OBSTACLE_DISTRIBUTION_LOG_PATH, "a") as f:
                timestamp = time.time()
                dist_str = ",".join(f"{val:.4f}" for val in distribution)
                f.write(f"{self.frame_number},{dist_str},{timestamp:.4f}\n")
        except Exception as e:
            self.log_error(f"Failed to save obstacle distribution to file: {e}")
